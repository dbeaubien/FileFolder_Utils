...
var $blob : Blob
RECEIVE PACKET:C104(fileBuffer_DocRef; $blob; 10)

If (fileBuffer_charSet="") && (BLOB size:C605($blob)>=4)
...
If (fileBuffer_charSet="") && (BLOB size:C605($blob)>=3)
	If ($blob{0}=239) && ($blob{1}=187) && ($blob{2}=191)  // starts with the UTF-8 BOM?
		fileBuffer_charSet:="UTF-8"
	End if 
End if 

If (fileBuffer_charSet="") && (BLOB size:C605($blob)>=2)
