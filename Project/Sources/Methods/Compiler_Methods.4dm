//%attributes = {"invisible":true}


//UnitTest_RunTestCase
C_TEXT:C284(UnitTest_RunTestCase; $1)

//UnitTest_AddTestCase
C_TEXT:C284(UnitTest_AddTestCase; $1)

//FileBuffer_GetFilePostion
C_LONGINT:C283(FileBuffer_GetFilePostion; $0)

//UnitTest_LogMessage
C_TEXT:C284(UnitTest_LogMessage; $1)

//STR_ParseLineToCsvValues
C_COLLECTION:C1488(STR_ParseLineToCsvValues; $0)
C_TEXT:C284(STR_ParseLineToCsvValues; $1)
C_TEXT:C284(STR_ParseLineToCsvValues; $2)

//STR_GetLeftOfChar
C_TEXT:C284(STR_GetLeftOfChar; $0)
C_TEXT:C284(STR_GetLeftOfChar; $1)
C_TEXT:C284(STR_GetLeftOfChar; $2)

//File_IsPNG
C_BOOLEAN:C305(File_IsPNG; $0)
C_TEXT:C284(File_IsPNG; $1)

//STR_IsOneOf
C_BOOLEAN:C305(STR_IsOneOf; $0)
C_TEXT:C284(STR_IsOneOf; ${2})
C_TEXT:C284(STR_IsOneOf; $1)

//FileBuffer_FetchData_ByString
C_TEXT:C284(FileBuffer_FetchData_ByString; $0)
C_TEXT:C284(FileBuffer_FetchData_ByString; $1)
C_TEXT:C284(FileBuffer_FetchData_ByString; $2)

//Folder_VerifyExistance
C_TEXT:C284(Folder_VerifyExistance; $1)

//UnitTest_AssertNotNil
C_POINTER:C301(UnitTest_AssertNotNil; $1)
C_TEXT:C284(UnitTest_AssertNotNil; $2)

//PTR_IsArray
C_BOOLEAN:C305(PTR_IsArray; $0)
C_POINTER:C301(PTR_IsArray; $1)

//File_DeriveMimeTypeFromName
C_TEXT:C284(File_DeriveMimeTypeFromName; $0)
C_TEXT:C284(File_DeriveMimeTypeFromName; $1)

//UnitTest_AssertEqualDate
C_DATE:C307(UnitTest_AssertEqualDate; $1)
C_DATE:C307(UnitTest_AssertEqualDate; $2)
C_TEXT:C284(UnitTest_AssertEqualDate; $3)

//Folder_ParentName
C_TEXT:C284(Folder_ParentName; $0)
C_TEXT:C284(Folder_ParentName; $1)

//UnitTest__Stopwatch
C_TEXT:C284(UnitTest__Stopwatch; $1)

//UnitTest_AssertEqualArray
C_POINTER:C301(UnitTest_AssertEqualArray; $1)
C_POINTER:C301(UnitTest_AssertEqualArray; $2)
C_TEXT:C284(UnitTest_AssertEqualArray; $3)

//UnitTest_AssertEqualLongint
C_LONGINT:C283(UnitTest_AssertEqualLongint; $1)
C_LONGINT:C283(UnitTest_AssertEqualLongint; $2)
C_TEXT:C284(UnitTest_AssertEqualLongint; $3)

//UnitTest_AssertNil
C_POINTER:C301(UnitTest_AssertNil; $1)
C_TEXT:C284(UnitTest_AssertNil; $2)

//File_Position
C_LONGINT:C283(File_Position; $0)
C_TEXT:C284(File_Position; $1)
C_TIME:C306(File_Position; $2)
C_LONGINT:C283(File_Position; $3)

//UnitTest_SaveStats
C_TEXT:C284(UnitTest_SaveStats; $1)

//File__UnitTests
C_TEXT:C284(File__UnitTests; $1)

//File_IsJPG
C_BOOLEAN:C305(File_IsJPG; $0)
C_TEXT:C284(File_IsJPG; $1)

//UnitTest_AssertEqualText
C_TEXT:C284(UnitTest_AssertEqualText; $1)
C_TEXT:C284(UnitTest_AssertEqualText; $2)
C_TEXT:C284(UnitTest_AssertEqualText; $3)

//File_DoesExist
C_BOOLEAN:C305(File_DoesExist; $0)
C_TEXT:C284(File_DoesExist; $1)

//File_GetFileName
C_TEXT:C284(File_GetFileName; $0)
C_TEXT:C284(File_GetFileName; $1)

//File_GetFolderName
C_TEXT:C284(File_GetFolderName; $0)
C_TEXT:C284(File_GetFolderName; $1)

//FileBuffer_FetchTabDelimitedLne
C_TEXT:C284(FileBuffer_FetchTabDelimitedLne; $1)
C_POINTER:C301(FileBuffer_FetchTabDelimitedLne; $2)

//FileBuffer_FetchData_PeekAhead
C_TEXT:C284(FileBuffer_FetchData_PeekAhead; $0)
C_LONGINT:C283(FileBuffer_FetchData_PeekAhead; $1)

//UTF8_GetByteCountFrom1stChar
C_LONGINT:C283(UTF8_GetByteCountFrom1stChar; $0)
C_LONGINT:C283(UTF8_GetByteCountFrom1stChar; $1)

//File_IsTabDelimited
C_BOOLEAN:C305(File_IsTabDelimited; $0)
C_TEXT:C284(File_IsTabDelimited; $1)

//File_IsCSV
C_BOOLEAN:C305(File_IsCSV; $0)
C_TEXT:C284(File_IsCSV; $1)

//Array_Empty
C_POINTER:C301(Array_Empty; $1)

//FileBuffer_TellMeTheEOL
C_TEXT:C284(FileBuffer_TellMeTheEOL; $0)

//Folder_DoesExist
C_BOOLEAN:C305(Folder_DoesExist; $0)
C_TEXT:C284(Folder_DoesExist; $1)

//FileBuffer_EOF
C_BOOLEAN:C305(FileBuffer_EOF; $0)

//File_IsDelimited
C_BOOLEAN:C305(File_IsDelimited; $0)
C_TEXT:C284(File_IsDelimited; $1)
C_TEXT:C284(File_IsDelimited; $2)

//UnitTest_AssertEqualTime
C_TIME:C306(UnitTest_AssertEqualTime; $1)
C_TIME:C306(UnitTest_AssertEqualTime; $2)
C_TEXT:C284(UnitTest_AssertEqualTime; $3)

//CSV_LineIsComplete
C_BOOLEAN:C305(CSV_LineIsComplete; $0)
C_TEXT:C284(CSV_LineIsComplete; $1)
C_TEXT:C284(CSV_LineIsComplete; $2)

//UnitTest_AssertEqualPointer
C_POINTER:C301(UnitTest_AssertEqualPointer; $1)
C_POINTER:C301(UnitTest_AssertEqualPointer; $2)
C_TEXT:C284(UnitTest_AssertEqualPointer; $3)

//STR_TellMeTheEOL
C_TEXT:C284(STR_TellMeTheEOL; $0)
C_TEXT:C284(STR_TellMeTheEOL; $1)

//FileBuffer_Init
C_TIME:C306(FileBuffer_Init; $1)
C_LONGINT:C283(FileBuffer_Init; $2)
C_TEXT:C284(FileBuffer_Init; $3)

//FileBuffer_FetchDelimitedLne
C_TEXT:C284(FileBuffer_FetchDelimitedLne; $1)
C_POINTER:C301(FileBuffer_FetchDelimitedLne; $2)
C_TEXT:C284(FileBuffer_FetchDelimitedLne; $3)

//UnitTest_AssertArraySize
C_LONGINT:C283(UnitTest_AssertArraySize; $1)
C_POINTER:C301(UnitTest_AssertArraySize; $2)
C_TEXT:C284(UnitTest_AssertArraySize; $3)

//UnitTest_AssertFalse
C_BOOLEAN:C305(UnitTest_AssertFalse; $1)
C_TEXT:C284(UnitTest_AssertFalse; $2)

//UnitTest_AssertRecordCount
C_LONGINT:C283(UnitTest_AssertRecordCount; $1)
C_POINTER:C301(UnitTest_AssertRecordCount; $2)
C_TEXT:C284(UnitTest_AssertRecordCount; $3)

//FileBuffer_FetchData_BySize
C_TEXT:C284(FileBuffer_FetchData_BySize; $0)
C_LONGINT:C283(FileBuffer_FetchData_BySize; $1)

//UnitTest_AssertFolderExists
C_TEXT:C284(UnitTest_AssertFolderExists; $1)
C_TEXT:C284(UnitTest_AssertFolderExists; $2)

//File_Delete
C_TEXT:C284(File_Delete; $1)

//UnitTest_AssertFileExists
C_TEXT:C284(UnitTest_AssertFileExists; $1)
C_TEXT:C284(UnitTest_AssertFileExists; $2)

//UnitTest_RunTest
C_TEXT:C284(UnitTest_RunTest; $1)

//UnitTest__ResolvePointer
C_TEXT:C284(UnitTest__ResolvePointer; $0)
C_POINTER:C301(UnitTest__ResolvePointer; $1)

//FileBuffer__UnitTests
C_TEXT:C284(FileBuffer__UnitTests; $1)

//UnitTest_AssertEqualTextAndCase
C_TEXT:C284(UnitTest_AssertEqualTextAndCase; $1)
C_TEXT:C284(UnitTest_AssertEqualTextAndCase; $2)
C_TEXT:C284(UnitTest_AssertEqualTextAndCase; $3)

//CallChain_FormatPretty
C_TEXT:C284(CallChain_FormatPretty; $0)
C_COLLECTION:C1488(CallChain_FormatPretty; $1)

//UnitTest_Assert
C_BOOLEAN:C305(UnitTest_Assert; $1)
C_TEXT:C284(UnitTest_Assert; $2)

//UnitTest_Init
C_TEXT:C284(UnitTest_Init; $1)

//UnitTest_AssertEqualReal
C_REAL:C285(UnitTest_AssertEqualReal; $1)
C_REAL:C285(UnitTest_AssertEqualReal; $2)
C_TEXT:C284(UnitTest_AssertEqualReal; $3)

//FileBuffer_FetchCSVLine
C_TEXT:C284(FileBuffer_FetchCSVLine; $1)
C_POINTER:C301(FileBuffer_FetchCSVLine; $2)

//UnitTest_SaveLog
C_TEXT:C284(UnitTest_SaveLog; $1)
C_POINTER:C301(File_ImportTabDelimited2Arrays; ${2})
C_TEXT:C284(File_ImportTabDelimited2Arrays; $1)

//File_GetExtension
C_TEXT:C284(File_GetExtension; $0)
C_TEXT:C284(File_GetExtension; $1)

//UnitTest_AssertTrue
C_BOOLEAN:C305(UnitTest_AssertTrue; $1)
C_TEXT:C284(UnitTest_AssertTrue; $2)

//Array_ConvertFromTextDelimited
C_POINTER:C301(Array_ConvertFromTextDelimited; $1)
C_TEXT:C284(Array_ConvertFromTextDelimited; $2)
C_TEXT:C284(Array_ConvertFromTextDelimited; $3)

//Folder_EnsureEndsInSeparator
C_TEXT:C284(Folder_EnsureEndsInSeparator; $0)
C_TEXT:C284(Folder_EnsureEndsInSeparator; $1)

//CSV_LineFromFieldCollection
C_TEXT:C284(CSV_LineFromFieldCollection; $0)
C_COLLECTION:C1488(CSV_LineFromFieldCollection; $1)
C_TEXT:C284(CSV_LineFromFieldCollection; $2)

//Folder_isOnMountedVolume
C_BOOLEAN:C305(Folder_isOnMountedVolume; $0)
C_TEXT:C284(Folder_isOnMountedVolume; $1)

//File_DecompressIt
C_TEXT:C284(File_DecompressIt; $1)
C_TEXT:C284(File_DecompressIt; $2)

//Folder_EmptyContents
C_TEXT:C284(Folder_EmptyContents; $1)

//CSV_LineFromCollection
C_TEXT:C284(CSV_LineFromCollection; $0)
C_COLLECTION:C1488(CSV_LineFromCollection; $1)
C_TEXT:C284(CSV_LineFromCollection; $2)

//Folder_RemoveDotFileFromArray
C_POINTER:C301(Folder_RemoveDotFileFromArray; $1)

//Folder_Delete
C_TEXT:C284(Folder_Delete; $1)

//Folder_VerifyItExists
C_TEXT:C284(Folder_VerifyItExists; $1)
C_POINTER:C301(File_ExportArrays2TabDelimited; ${2})
C_TEXT:C284(File_ExportArrays2TabDelimited; $1)

//File_VerifyitExist
C_BOOLEAN:C305(File_VerifyitExist; $0)
C_TEXT:C284(File_VerifyitExist; $1)

//File_CreateFile
C_TIME:C306(File_CreateFile; $0)
C_TEXT:C284(File_CreateFile; $1)
C_TEXT:C284(File_CreateFile; $2)

//File_WaitUntilDoesExist
C_BOOLEAN:C305(File_WaitUntilDoesExist; $0)
C_TEXT:C284(File_WaitUntilDoesExist; $1)
C_LONGINT:C283(File_WaitUntilDoesExist; $2)

//Folder_RemoveOlderContents
C_TEXT:C284(Folder_RemoveOlderContents; $1)
C_LONGINT:C283(Folder_RemoveOlderContents; $2)

//File_GenerateUniqueName
C_TEXT:C284(File_GenerateUniqueName; $0)

//Folder_CompressIt
C_TEXT:C284(Folder_CompressIt; $1)
C_TEXT:C284(Folder_CompressIt; $2)
C_BOOLEAN:C305(Folder_CompressIt; $3)

//File_WaitUntilFixedSize
C_BOOLEAN:C305(File_WaitUntilFixedSize; $0)
C_TEXT:C284(File_WaitUntilFixedSize; $1)
C_LONGINT:C283(File_WaitUntilFixedSize; $2)
C_LONGINT:C283(File_WaitUntilFixedSize; $3)

//File_GetChecksum
C_TEXT:C284(File_GetChecksum; $0)
C_TEXT:C284(File_GetChecksum; $1)
C_TEXT:C284(File_GetChecksum; $2)

//File_ExportTableSelection2CSV
C_OBJECT:C1216(File_ExportTableSelection2CSV; $0)
C_POINTER:C301(File_ExportTableSelection2CSV; $1)
C_COLLECTION:C1488(File_ExportTableSelection2CSV; $2)
C_TEXT:C284(File_ExportTableSelection2CSV; $3)

//File_Import2Collection
C_COLLECTION:C1488(File_Import2Collection; $0)
C_OBJECT:C1216(File_Import2Collection; $1)

//File_CopyTo4DServer
C_LONGINT:C283(File_CopyTo4DServer; $0)
C_TEXT:C284(File_CopyTo4DServer; $1)
C_TEXT:C284(File_CopyTo4DServer; $2)

//File_GetTailAsText
C_TEXT:C284(File_GetTailAsText; $0)
C_TEXT:C284(File_GetTailAsText; $1)
C_LONGINT:C283(File_GetTailAsText; $2)

//File_DownloadFromURL
C_TEXT:C284(File_DownloadFromURL; $1)
C_TEXT:C284(File_DownloadFromURL; $2)

//CSV__UnitTests
C_TEXT:C284(CSV__UnitTests; $1)

//File_GetHeadersFromFirstRow
C_TEXT:C284(File_GetHeadersFromFirstRow; $1)
C_POINTER:C301(File_GetHeadersFromFirstRow; $2)

//Folder_CopyFolderMoveFiles
C_LONGINT:C283(Folder_CopyFolderMoveFiles; $0)
C_TEXT:C284(Folder_CopyFolderMoveFiles; $1)
C_TEXT:C284(Folder_CopyFolderMoveFiles; $2)

//File_CompressIt
C_TEXT:C284(File_CompressIt; $1)
C_TEXT:C284(File_CompressIt; $2)
C_POINTER:C301(File_ExportArrays2CSV; ${2})
C_TEXT:C284(File_ExportArrays2CSV; $1)

//OnErr_Install_Handler
C_TEXT:C284(OnErr_Install_Handler; $1)

//Date2String
C_TEXT:C284(Date2String; $0)
C_DATE:C307(Date2String; $1)
C_TEXT:C284(Date2String; $2)

//Boolean2String
C_TEXT:C284(Boolean2String; $0)
C_BOOLEAN:C305(Boolean2String; $1)
C_TEXT:C284(Boolean2String; $2)
C_TEXT:C284(Boolean2String; $3)

//File_DeriveFileTypeFromName
C_TEXT:C284(File_DeriveFileTypeFromName; $0)
C_TEXT:C284(File_DeriveFileTypeFromName; $1)

//Time2String
C_TEXT:C284(Time2String; $0)
C_TIME:C306(Time2String; $1)
C_TEXT:C284(Time2String; $2)

//TS_FromDateTime
C_LONGINT:C283(TS_FromDateTime; $0)
C_DATE:C307(TS_FromDateTime; $1)
C_TIME:C306(TS_FromDateTime; $2)

//String2Date
C_DATE:C307(String2Date; $0)
C_TEXT:C284(String2Date; $1)
C_TEXT:C284(String2Date; $2)

//STR_TrimExcessQuotes
C_TEXT:C284(STR_TrimExcessQuotes; $0)
C_TEXT:C284(STR_TrimExcessQuotes; $1)

//NUM_GetMaxLong
C_LONGINT:C283(NUM_GetMaxLong; $0)
C_LONGINT:C283(NUM_GetMaxLong; $1)
C_LONGINT:C283(NUM_GetMaxLong; $2)

//STR_TrimExcessSpaces
C_TEXT:C284(STR_TrimExcessSpaces; $0)
C_TEXT:C284(STR_TrimExcessSpaces; $1)

//STR_isDate
C_BOOLEAN:C305(STR_isDate; $0)
C_TEXT:C284(STR_isDate; $1)

//STR_isDate_GetDate
C_DATE:C307(STR_isDate_GetDate; $0)
C_TEXT:C284(STR_isDate_GetDate; $1)

//STR_isISODate
C_BOOLEAN:C305(STR_isISODate; $0)
C_TEXT:C284(STR_isISODate; $1)

//STR_TrimExcessCharacter
C_TEXT:C284(STR_TrimExcessCharacter; $0)
C_TEXT:C284(STR_TrimExcessCharacter; $1)
C_TEXT:C284(STR_TrimExcessCharacter; $2)

//STR_isIntegerNumber
C_BOOLEAN:C305(STR_isIntegerNumber; $0)
C_TEXT:C284(STR_isIntegerNumber; $1)

//STR_isDate_MMDDYYYY
C_BOOLEAN:C305(STR_isDate_MMDDYYYY; $0)
C_TEXT:C284(STR_isDate_MMDDYYYY; $1)

//FileOnServer_GetMD5
C_TEXT:C284(FileOnServer_GetMD5; $0)
C_TEXT:C284(FileOnServer_GetMD5; $1)

//NUM_IsOneOf
C_BOOLEAN:C305(NUM_IsOneOf; $0)
C_LONGINT:C283(NUM_IsOneOf; ${2})
C_LONGINT:C283(NUM_IsOneOf; $1)

//FileOnServer_DoesExist
C_BOOLEAN:C305(FileOnServer_DoesExist; $0)
C_TEXT:C284(FileOnServer_DoesExist; $1)

//FileOnServer_GetDocSize
C_LONGINT:C283(FileOnServer_GetDocSize; $0)
C_TEXT:C284(FileOnServer_GetDocSize; $1)

//FileOnServer_AppendData
C_LONGINT:C283(FileOnServer_AppendData; $0)
C_TEXT:C284(FileOnServer_AppendData; $1)
C_BLOB:C604(FileOnServer_AppendData; $2)

//FileOnServer_Delete
C_TEXT:C284(FileOnServer_Delete; $1)