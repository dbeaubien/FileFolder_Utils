...
If (Asserted:C1132(Count parameters:C259=2))
	...
	C_LONGINT:C283($pos)
	C_BOOLEAN:C305($vb_atEOL;$vb_doneValue)
		...
		If ($nextChar#"")
			...
			Else 
				...
				If ($nextChar=Char:C90(Double quote:K15:41))  // We have a value that is quote delimited, scan until next quote.
					...
					Until ($vb_doneValue) | (FileBuffer_EOF )
					
					  //$theValue:=Substring($theValue;1;Length($theValue)-1)  // strip off the trailing quote
					  //$theValue:=Replace string($theValue;Char(Double quote)+Char(Double quote);Char(Double quote))  // re-convert any embedded quotes back to a single
